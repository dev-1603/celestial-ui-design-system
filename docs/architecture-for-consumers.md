...
The official framework bindings (e.g., `@celestial-ui/react`) are an _optional_ top-level layer (currently in development) that wrap the foundation for convenience. They are not required to use Celestial UI.
