...
describe('Token Resolution', () => {
  it('should flatten and resolve basic aliases', () => {
    const config: TokenConfig = {
      color: {
        blue: { 500: { $type: 'color', $value: '#3B82F6', $extensions: { celestial: { layer: 'primitive' } } } }
      },
      action: {
        primary: { $type: 'color', $value: '{color.blue.500}', $extensions: { celestial: { layer: 'semantic' } } }
      }
    };
    ...
    const config: TokenConfig = {
      ...
      shadow: {
        sm: {
          $type: 'shadow',
          $value: [
            { offsetX: '0px', offsetY: '1px', blur: '2px', spread: '0px', color: '{color.shadowBase}' }
          ]
        }
      }
    };
