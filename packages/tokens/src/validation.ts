  ...
  for (const [key, token] of Object.entries(resolvedTokens)) {
    ...
    if (extensions?.a11ySensitive && extensions?.contrastPairs) {
      ...
      for (const bgKey of extensions.contrastPairs) {
        ...
        if (!bgToken) {
          report.errors.push(`A11y violation: Token '${key}' specifies contrast pair '${bgKey}' which does not exist.`);
          report.isValid = false;
          continue;
        }
        ...
        try {
          ...
          if (!meetsContrastAA(ratio)) {
            report.isValid = false;
            report.errors.push(`A11y violation: '${key}' (${fgHex}) and '${bgKey}' (${bgHex}) do not meet WCAG 2.2 AA (Ratio: ${ratio.toFixed(2)}:1)`);
          }
        } catch (e: any) {
